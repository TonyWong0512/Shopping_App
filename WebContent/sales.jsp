...
<body>
    ...
    <%
        ...
        if (action != null && action.equals("yes") && rowsView.equals("Customer")) {
			...
			String customerQ = "SELECT ppc.username FROM (SELECT username, totalCost " + colClause1 + " FROM ProductsPerCustomers) as ppc " + whereClause1 + " GROUP BY ppc.username "+ groupClause1 +" ORDER BY SUM(totalCost) DESC LIMIT 10";
			String productsQ = "SELECT name FROM (SELECT name, quantity " + colClause2 +" FROM ProductsPerCustomers) as lol" + whereClause2 + " GROUP BY name " + groupClause2 + " ORDER BY SUM(quantity) DESC LIMIT 10";
